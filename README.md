...
#### Building DataSpace and the Open Access Repository

##### Cloning from GitLab
```bash
cd docker/usr/local/src
git clone https://git.atmire.com/clients/princeton-5.git dspace-5.5-src-release
cd dspace-5.5-src-release
git checkout dataspace-docker-build # For DataSpace
git checkout oar-docker-build # For the Open Access Repository (OAR)
```

##### Adding the Symplectic Module for the Open Access Repository
...
```bash
cd docker/usr/local/src/dspace-5.5-src-release
cp rt4ds.tar.gz .
...
#### Building the DSpace 5.5 Community Release
```bash
cd docker/usr/local/src
